...
class Generator(nn.Module):
    ...
    def __init__(self, large_kernel_size=9, small_kernel_size=3, in_channels=1, n_channels=64, n_blocks=16, scaling_factor=4):
        """
        :param large_kernel_size: kernel size of the first and last convolutions which transform the inputs and outputs
        :param small_kernel_size: kernel size of all convolutions in-between, i.e. those in the residual and subpixel convolutional blocks
        :param n_channels: number of channels in-between, i.e. the input and output channels for the residual and subpixel convolutional blocks
        ...
        # The generator is simply a ResNet, as above
        self.net = ResNet(large_kernel_size=large_kernel_size, small_kernel_size=small_kernel_size,
                            in_channels=in_channels, n_channels=n_channels, n_blocks=n_blocks)

    def initialize_with_resnet(self, resnet_checkpoint):
        """
        Initialize with weights from a trained SRResNet.
        :param resnet_checkpoint: checkpoint filepath
        """
        resnet = torch.load(resnet_checkpoint)['resnet']
        self.net.load_state_dict(resnet.state_dict())

        print("\nLoaded weights from pre-trained ResNet.\n")

    def forward(self, imgs):

        out_imgs = self.net(imgs)  # (N, n_channels, w, h)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, kernel_size=3, in_channel=1, n_channels=64, n_blocks=8, fc_size=1024):
        """
